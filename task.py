...
class Task:
    def __init__(self, name, account, day, start_time, priority = 'low', now = False):
        ...
        self.next_start_time = self.get_next_time(start_time, day, now)
    ...
    def get_next_time(self, start_time, day, now):
        if now:
            next_time = 0
        else:
            next_time = self.load_config(key = 'next_time')
            if next_time is None:
                n = time.time()
                today_start_time = time.mktime(time.strptime('%s %s' % (datetime.now().strftime('%y%m%d'), start_time), '%y%m%d %H:%M:%S'))
                if n > today_start_time:
                    next_time = today_start_time + 24*3600  # tomorrow
                else:
                    next_time = today_start_time
        return next_time
    ...
    def load_config(self, key = None):
        ...
        if not self.name in self.ini.sections():
            ...
            return None
        items = self.ini.items(self.name)
        if key:
    ...
    def save_config(self, key, value):
        if os.path.isfile(self.ini_file): self.ini.read(self.ini_file)
        self.ini.set(self.name, key, value)
